...
- [x] **Task 0.1** 注册 12 个缺失页面到 `app.config.ts`
...
- [x] **Task 0.2** 补充 `communityApi.getCircles / getCircleDetail / joinCircle / leaveCircle`
  - 文件：`src/domains/community/api.ts`

- [x] **Task 0.3** 补充 `marketingApi.getReferralInfo` + `ReferralInfo` 类型
...
- [x] **Task 2.1** 地址上限检查（20 个上限）
- [x] **Task 2.2** 物流增强（异常提醒 + 多包裹 + 扫码录入）
- [x] **Task 2.3** 领取优惠券功能（`claimCoupon / getCouponTemplates`）
- [x] **Task 2.4** 创作者中心增强（达人认证 + 佣金数据 + 晒单）
- [x] **Task 2.5** 分享增强（朋友圈分享 + Canvas 降级）
- [x] **Task 2.6** 管理后台完善（独立分包 + 趋势图）
- [x] **Task 2.7** 出价到期提醒（倒计时 + 颜色提醒）
- [x] **Task 2.8** 积分抵扣 UI（开关 + 换算 + 上限 50%）

## Phase 3: 质量收尾（P3）

- [x] **Task 3.1** i18n 硬编码清理（44 文件 377 处）
- [x] **Task 3.2** 余额隐藏/显示切换
- [x] **Task 3.3** 发帖草稿自动保存
